...
for pcontent in range(len(article_href)):
    ...
    z=con.find("哪")
    a=con.find("ψ")
    PTTCOURSE_dct["哪一學年度修課"]=con[z:a].replace('\n'," ").replace("哪一學年度修課：","")
    b=con.find("λ")
    PTTCOURSE_dct["授課教師"] = con[a:b].replace('\n'," ").replace("ψ 授課教師 (若為多人合授請寫開課教師，以方便收錄) ","")
    c=con.find("δ")
    PTTCOURSE_dct["開課系所與授課對象"] = con[b:c].replace('\n'," ").replace("λ 開課系所與授課對象 (是否為必修或通識課 / 內容是否與某些背景相關)  ","")
    d=con.find("Ω")
    PTTCOURSE_dct["課程大概內容"] = con[c:d].replace('\n'," ").replace("δ 課程大概內容 ","")
    e=con.find("η")
    PTTCOURSE_dct["私心推薦指數"] = con[d:e].replace('\n'," ").replace("Ω 私心推薦指數(以五分計) ","")
    f=con.find("μ")
    PTTCOURSE_dct["上課用書"] = con[e:f].replace('\n'," ").replace("η 上課用書(影印講義或是指定教科書) ","")
    g=con.find("σ")
    PTTCOURSE_dct["上課方式"] = con[f:g].replace('\n'," ").replace("μ 上課方式(投影片、團體討論、老師教學風格) ","")
    h=con.find("ρ")
    PTTCOURSE_dct["評分方式"] = con[g:h].replace('\n'," ").replace("σ 評分方式(給分甜嗎？是紮實分？) ","")
    j=con.find("ω")
    PTTCOURSE_dct["考題型式作業方式"] = con[h:j].replace('\n'," ").replace("ρ 考題型式、作業方式：","")
    k=con.find("Ψ")
    PTTCOURSE_dct["其它"] = con[j:k].replace('\n'," ").replace("ω 其它(是否注重出席率？如果為外系選修，需先有什麼基礎較好嗎？老師個性？ 加簽習慣？嚴禁遲到等…) ","")
    l=con.find("--")
    PTTCOURSE_dct["總結"]=con[k:l].replace('\n'," ").replace("Ψ 總結 ","")
    #print(PTTCOURSE_dct)
    #
    comentment=[]
